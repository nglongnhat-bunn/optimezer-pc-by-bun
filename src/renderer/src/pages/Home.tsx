...
function Home() {
  ...
  const [tweakInfo, setTweakInfo] = useState(() => {
    try {
      const cached = localStorage.getItem("longnhat:tweakInfo")
  ...
  const [activeTweaks, setActiveTweaks] = useState(() => {
    try {
      const cached = localStorage.getItem("longnhat:activeTweaks")
  ...
  const fetchActiveTweaks = async () => {
    try {
      ...
      localStorage.setItem("longnhat:activeTweaks", JSON.stringify(active))
    } catch (err) {
  ...
  useEffect(() => {
    const idleHandle = requestIdleCallback(() => {
      const cached = localStorage.getItem("longnhat:systemInfo")
      if (cached) {
        try {
      ...
      invoke({ channel: "get-system-info" })
        .then((info) => {
          useSystemStore.setState((state) => {
            ...
            localStorage.setItem("longnhat:systemInfo", JSON.stringify(merged))
  ...
  useEffect(() => {
    const idleHandle = requestIdleCallback(() => {
      ...
      invoke({ channel: "tweaks:fetch" })
        .then((tweaks) => {
          setTweakInfo(tweaks)
          localStorage.setItem("longnhat:tweakInfo", JSON.stringify(tweaks))
        })
        .catch((err) => {
      ...
      <RootDiv>
        <div className="flex items-center justify-center h-64 flex-col gap-4">
          ...
          <p className="text-xs text-sparkle-secondary bg-sparkle-accent px-3 py-1.5 rounded-full mt-1">
            You can use other parts of LongNhat while this loads
          </p>
        </div>
      </RootDiv>
    )
