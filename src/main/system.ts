...
export async function getAdminStatus(): Promise<boolean> {
  console.log("[LongNhat]: Checking admin status...")
  try {
    const { execSync } = await import("child_process")
    execSync("net session", { stdio: "pipe" })
    console.log("[LongNhat]: Admin status: true")
    return true
  } catch (error) {
    console.log("[LongNhat]: Not running as admin")
    return false
  }
}
function clearSparkleCache(): ClearCacheResult {
  ...
  try {
    ...
    if (fs.existsSync(scriptsPath)) {
      ...
      scriptsCleared = true
      console.log("LongNhat scripts directory files cleared successfully.")
    } else {
      console.warn("LongNhat scripts directory does not exist.")
    ...
    if (fs.existsSync(logsPath)) {
      ...
      logsCleared = true
      console.log("LongNhat logs directory files cleared successfully.")
    } else {
      console.warn("LongNhat logs directory does not exist.")
    ...
    console.error("Failed to clear LongNhat scripts or logs directory:", error)
    return { success: false, error: error.message }
  }
}
...
function Show-InstallerGUI {
    ...
    $label.Text = "Welcome! LongNhat needs Winget to install apps."
    $label.AutoSize = $true
...
if ($TestMode -or -not (Check-Winget)) {
    ...
    Write-Output "Winget is already installed. LongNhat can install apps!"
}
`
...
export const setupSystemHandlers = (): void => {
  ...
  ipcMain.handle("clear-longnhat-cache", clearSparkleCache)
...
export const cleanupSystemHandlers = (): void => {
  ...
  ipcMain.removeHandler("clear-longnhat-cache")
